...
(() => {
  ...
  if (form) {
    form.addEventListener('submit', (event) => {
      ...
      window.location.href = `mailto:softzenotech@gmail.com?subject=New Project Inquiry - Softzeno Tech&body=${body}`;

      setTimeout(() => {
        if (status) status.textContent = 'If your email app did not open, message us directly on WhatsApp.';
      }, 1100);
    });
  }
})();
...
(() => {
  ...
  const setTab = (key) => {
    const item = data[key];
    if (!item) return;
    card.setAttribute("data-showcase", key);
